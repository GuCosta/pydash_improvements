...
#   Implementação do algoritmo fuzzy  
...
class r2a_fuzzy(IR2A):
    ...
    def handle_segment_size_request(self, msg):
        ...
        if len(self.whiteboard.get_playback_segment_size_time_at_buffer()) > 0:
            ...
            # Desenvolvimento 
            # print("\nBUFFERING TIME ----------->", buffering_time)
            # print("\nDIF BUFFERING TIME ------->", differential_buffering_time)
            # print("\nF -----------------------> ", f)
            # print("\nLAST QI: ", next_qi)
            # print('\nSELECTED QI: ', last_qi)
            # print('\nMOVING AVG THROU: ', moving_average_throughput)
            # print("\nESTIMATIVA NOVO   --------> ", predicted_buffer_new_qi)
            # print("\nESTIMATIVA ANTIGA --------> ", predicted_buffer_old_qi)
            # print("\nQUANTIDADE DE PAUSAS: ", RED+str(len(self.whiteboard.get_playback_pauses()))+RESET+"\n")
